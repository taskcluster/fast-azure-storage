...
function authorizeWithRefreshSAS(method, path, query, headers) {
  ...
  if (Date.now() > this._nextSASRefresh && this._nextSASRefresh !== 0) {
    ...
    this._sas.then(function(sas) {
      ...
      self._nextSASRefresh = new Date(sas.se).getTime()
                             - self.options.minSASAuthExpiry;
 ...
 *   timeout:              30 * 1000,
 *
 *   // Set meta-data level for response (use full to get eTag in queryEntities)
...
function Table(options) {
  ...
  // Compute server side timeout
  this.timeout = Math.round(this.options.timeout / 1000);
 ...
 * `https.request`.
 */
Table.prototype.authorize = function() {
 ...
 * exist.
 *
 * If **`options.eTag` is a string** (other than `'*'`) the remote entity will
 * be replaced or merged depending on `mode`, if the ETag of the remote entity
