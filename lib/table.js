...
function Table(options) {
  ...
  // Construct hostname
  this._hostname  = options.credentials.accountId + '.table.core.windows.net';
  // Decode accessKey
...
Table.prototype.sas = function(table, options) {
  ...
  assert(options.end instanceof Date,
         "options.end must be a Date object");
  ...
  var query = {
    sv:   '2014-02-14', // sas signature version, try 2013-08-15
    tn:   table,
    se:   options.expiry.toJSON(),
    sp:   permissions,
    sig:  null
  };
...
Table.prototype.request = function request(method, path, query, headers, json) {
  ...
  var linesToSign = [
    ...
    headers['x-ms-date']            // we always include x-ms-date
  ];
  ...
  var canonicalizedResource = '/' + accountId + path;
  if (query.comp !== undefined) {
    canonicalizedResource += '?comp=' + query.comp;
  }
  linesToSign.push(canonicalizedResource);
