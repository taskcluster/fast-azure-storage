 ...
 * @const
 */
utils.TRANSIENT_HTTP_ERROR_CODES = [
  'ETIMEDOUT',
  'ECONNRESET',
  'EADDRINUSE',
  'ESOCKETTIMEDOUT',
 ...
 */
utils.retry = function retry(f, options) {
  var retry = 0;
  function attempt() {
    return f(retry).catch(function(err) {
      ...
      return utils.sleep(delay).then(function() {
        return attempt();
      });
    });
  };
 ...
 */
utils.request = function(options, data) {
 ...
 */
utils.dateToISOWithoutMS = function(date) {
